...
@router.get('/')
def get_users(request: Request, db: Session = Depends(get_db)) -> list[User]:
    response = users.get_list_users(db=db)
    return response


@router.get('/{user_id}/')
def get_user(request: Request, user_id: int, db: Session = Depends(get_db)) -> User:
    response = users.get_user(user_id, db)
    return response


@router.post('/')
def add_user(request: Request, user_data: CreateUser, db: Session = Depends(get_db)) -> User:
    response = users.create_user(user_data, db)
    return response


@router.patch('/{user_id}/')
def update_user(request: Request, user_data: UpdateUser, db: Session = Depends(get_db)) -> User:
