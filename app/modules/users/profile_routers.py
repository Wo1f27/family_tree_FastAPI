...
@router.post('/')
def create_profile(user_data: CreateProfile, db: Session = Depends(get_db)) -> Profile:
    response = profiles.create_profile(user_data, db)
    return response


@router.patch('/{profile_id}/')
def update_profile(user_data: UpdateProfile, db: Session = Depends(get_db)) -> Profile:
